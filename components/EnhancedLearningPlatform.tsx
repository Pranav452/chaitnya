...
type Progress = Record<Language, number>;
...
const generateQuizQuestion = (language: Language): QuizQuestion => {
  const questions = {
    HTML: [
      {
...
export default function EnhancedLearningPlatform() {
  ...
  const handleQuizAnswer = async (answerIndex: number) => {
    if (quizQuestion && selectedLanguage && answerIndex === quizQuestion.correctAnswer) {
  ...
  const sendChatMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (chatInput.trim()) {
      // Correctly typing the new message as 'user' or 'bot' to match the expected type
      const newMessages = [...chatMessages, { text: chatInput, sender: 'user' as 'user' | 'bot' }];
      setChatMessages(newMessages);
      setChatInput('');
      const response = await api.getChatbotResponse(chatInput);
      // Correctly typing the bot's response as 'bot' to match the expected type
      setChatMessages([...newMessages, { text: response, sender: 'bot' as 'user' | 'bot' }]);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {languages.map((language) => (
                <button
                  key={language.name}
                  onClick={() => {
                    setSelectedLanguage(language.name as Language);
        ...
        return (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {chapters[selectedLanguage || 'HTML'].map((chapter) => (
                <button
                  key={chapter}
